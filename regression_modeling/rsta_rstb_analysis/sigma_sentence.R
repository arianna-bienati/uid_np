...
min(sent_data$sigma_gamma)
# check how many sigma values are 0 or smaller
print("Number of sigma values <= 0:")
sum(sent_data$sigma_gamma <= 0, na.rm=TRUE)
# where are the 0 values
print("Location of 0 values:")
which(sent_data$sigma_gamma == 0)
sent_data$sigma_gamma[sent_data$sigma_gamma == 0]

# stop redirecting output to file
sink()

# adjust 0 values
#sent_data$sigma_adj <- ifelse(sent_data$sigma_gamma == 0, 1e-6, np_data$sigma_gamma)
...
cat("model_summary", output, file="sigma_sent_model.txt", sep="\n", append=TRUE)
...
pdf("sigma_sent_qqplot_residuals.pdf")
...
# effect of year
pdf("sigma_sent_effect_year.pdf")
...
# effect of journal
pdf("sigma_sent_effect_journal.pdf")
ggeffects::ggeffect(lm_sigma, c("journal_F")) %>%
  plot() +
  labs(x = "Journal",
       y = "Sigma",
       title = "")
...
pdf("sigma_sent_effect_sentLen.pdf")
ggeffects::ggeffect(lm_sigma, c("sent_len_c")) %>%
  plot() +
...
pdf("sigma_sent_effect_avgSrp.pdf")
...
write.csv(cv_results,
          file = "sigma_sent_cv_results.csv")
