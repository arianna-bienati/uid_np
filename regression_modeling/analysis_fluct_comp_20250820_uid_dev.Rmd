...
results <- future.apply::future_lapply(
  ...
  function(split) {
    ...
    fit <- glmmTMB::glmmTMB(uid_dev_shift ~ year_c * NP_len
                           + head_synt_role_F
                           + (1|head_lemma)
                           + (1|author),
                           data = train,
    ...
    dplyr::tibble(
      rmse = sqrt(mean((test$uid_dev_shift - preds)^2)),
      mae  = mean(abs(test$uid_dev_shift - preds))
    )
  },
  future.seed = TRUE,
  future.scheduling = 1,
...
```{r}
lm_uid_dev_final <- glmmTMB::glmmTMB(uid_dev ~ year_c * NP_len
                                     + head_synt_role_F
                                     + (1|head_lemma)
                                     + (1|author), 
                                     data = data_sample_2,
...
ggeffects::ggeffect(lm_uid_dev_final, c("year_c")) %>%
  plot() +
  labs(x = "Time",
       y = "Predicted Uniform Information Density (uid_dev)",
       title = "")
# plot effect of NP length
ggeffects::ggeffect(lm_uid_dev_final, c("NP_len")) %>%
  plot() +
  labs(x = "Number of Constituents",
       y = "Predicted Uniform Information Density (uid_dev)",
       title = "")
